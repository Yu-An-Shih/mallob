...
public:
    ...
    bool receive(int source, int tag, JobMessage& msg) {
        ...
        if (tag == MSG_JOB_TREE_BROADCAST) {
            _finished = true;
            _base_msg.payload = std::move(msg.payload);
        }
        return true;
    }
    
    bool receive_origin(int source, int tag, JobMessage& msg) {
    ...
    void advance() {
        ...
        if (!_aggregating && _future_aggregate.valid()) {
            ...
            if (_tree.isRoot()) {
                // Transform reduced element at root  /* Q: what is this for? */
